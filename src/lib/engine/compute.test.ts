...
describe("computeStat", () => {
  ...
  it("grouped: declaration order does not change the result", () => {
    ...
    const levels = { "test.srcA": 10, "test.srcC": 1 };
    expect(computeStat(reordered, levels, {})).toBeCloseTo(
      computeStat(mixedFormula, levels, {}),
    );
  });
  ...
  it("'×1+': multiplies by (1 + bonus fraction)", () => {
    ...
    expect(computeStat(bonusFormula, levels, {})).toBeCloseTo(
      (4 + 0.1) * (1 + 22),
    );
  });
  ...
  it("'=' replaces the base before adds and factors", () => {
    ...
    expect(computeStat(f, levels, { fish: 10 })).toBeCloseTo(
      (0.03 + 0.1) * 1.5,
    );
  });
...
describe("computeAll", () => {
  ...
  it("returns a result for every key in the FormulaMap", () => {
    ...
    expect(Object.keys(results)).toEqual(
      expect.arrayContaining(["stat_a", "stat_b"]),
    );
  });
