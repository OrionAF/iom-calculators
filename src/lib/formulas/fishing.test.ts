...
describe("fishingFormulas — super_shiny_chance", () => {
  ...
  it("T2 upgrade level 20 only → 0.20", () => {
    expect(
      computeStat(f, { "fishing.superShinyChanceT2": 20 }, {}),
    ).toBeCloseTo(0.2);
  });
...
describe("fishingFormulas — drone_multiplier", () => {
  ...
  it("Fishing With Friends level 3 → +0.30", () => {
    expect(
      computeStat(f, { "skillTree.fishingWithFriends": 3 }, {}),
    ).toBeCloseTo(1.3);
  });
});
...
describe("fishingFormulas — shiny_chance", () => {
  ...
  it("T1 upgrade max (25) → 0.125", () => {
    expect(computeStat(f, { "fishing.shinyChanceT1": 25 }, {})).toBeCloseTo(
      0.125,
    );
  });
...
describe("fishingFormulas — super_shiny_multi", () => {
  ...
  it("T2 enhance level 20 → 3 + 3.00 = 6.00", () => {
    expect(computeStat(f, { "fishing.superShinyMultiE2": 20 }, {})).toBeCloseTo(
      6.0,
    );
  });
});
...
describe("fishingFormulas — unknown contributions", () => {
  it("rod_power has NO unknowns — Half Way Bundle and card resolved", () => {
    expect(hasUnknownContributions(fishingFormulas["fishing_rod_power"])).toBe(
      false,
    );
  });
  ...
  it("super_shiny_chance has NO unknowns — all sources defined", () => {
    expect(
      hasUnknownContributions(fishingFormulas["fishing_super_shiny_chance"]),
    ).toBe(false);
  });

  it("token_multi has NO unknowns — only fishing sources", () => {
    expect(
      hasUnknownContributions(fishingFormulas["fishing_token_multi"]),
    ).toBe(false);
  });

  it("notice_requirement has NO unknowns — only skill tree source", () => {
    expect(
      hasUnknownContributions(fishingFormulas["fishing_notice_requirement"]),
    ).toBe(false);
  });
});
