...
function parseEntries(raw: string): {
  ...
  for (const part of parts) {
    ...
    const opMatches = [...p.matchAll(/<math>\s*(\+|\\times|=)\s*\??<\/math>|<math>\+\?<\/math>/g)]
    const lastOp = opMatches.length ? MATH_OP[opMatches[opMatches.length - 1][1] ?? '+'] : undefined
    const unknown =
      /<math>\s*\??\s*\+?\?\s*<\/math>|\{\{Edit button\|text=\?\}\}|<math>\?<\/math>/.test(p)
...
export function parseStatTemplate(body: string): WikiStat | null {
  ...
  for (let i = 3; i < fields.length; i++) {
    ...
    const header = f.match(/'''([^']+)'''\s*(?:<math>\s*(\+|\\times|=)\s*<\/math>)?/)
    if (!header) continue
