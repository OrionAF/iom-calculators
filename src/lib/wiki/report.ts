...
function formulaOpsBySystem(
  formula: StatFormula,
): Map<SourceSystem, Set<string>> {
...
export function buildWikiReport(readFile: (path: string) => string): string {
  ...
  for (const [name, wiki] of wikiStats) {
    ...
    const unknownCount = flattenContributions(formula).filter(
      (c) => c.unknown,
    ).length;
    ...
    for (const group of wiki.groups) {
      ...
      if (!covered) {
        const note =
          unknownCount > 0
            ? ` (formula has ${unknownCount} unknown placeholder(s))`
            : "";
        findings.push(
      ...
      const ourOps = new Set(
        mapped.flatMap((s) => [...(opsBySystem.get(s) ?? [])]),
      );
      if (
